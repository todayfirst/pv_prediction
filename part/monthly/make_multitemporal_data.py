...
def run(basic_data, Pbasic_data, test_mode, comsset,hours, minute, weather_ele, plant_id):
    ...
    if test_mode[0] == 1:  
        ...
        Ptraining_data.index = Pbasic_data.index
...
#        Ptraining_data["10sum"]=Pbasic_data["10sum"]
    ...
    if test_mode[0]==4:
        numofimage = len(training_data)
        start = round(numofimage/test_mode[2]*(test_mode[1]-1))
        end = round(numofimage/test_mode[2]*(test_mode[1]))
        
        Ptraining_data = training_data[start:end]
        training_data = pd.concat([training_data[:start],training_data[end:] ])
        '''
        training_data.reset_index(drop=True,inplace=True)
        training_data["coms_index"] = training_data.index
        Ptraining_data.reset_index(drop=True,inplace=True)
        Ptraining_data["coms_index"] = Ptraining_data.index
        '''
